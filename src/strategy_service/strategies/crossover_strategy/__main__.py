"""
Test module for MA Crossover Strategy
Run with: python -m src.strategy_service.strategies.crossover_strategy test
"""
...
def run_test():
    ...
    try:
        ...
        if len(signals[signals['signal'] != 0]) > 0:
            ...
            cols_to_show = ['date', 'close', 'signal']
            if 'signal_strength' in signals.columns:
                cols_to_show.append('signal_strength')
            print(signals[signals['signal'] != 0][cols_to_show].tail())
        
        print("\n" + "=" * 60)
        print("✅ MA Crossover Strategy test completed successfully!")
        return True
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "test":
        ...
        print("  python -m src.strategy_service.strategies.crossover_strategy test")
