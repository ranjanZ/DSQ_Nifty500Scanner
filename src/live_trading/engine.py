...
class LiveTradingEngine:
    ...
    def scan_for_signals_with_history(self, days_back: int = 100) -> List[Dict]:
        ...
        Matches backtest_offline logic exactly:
        - Uses last 5 rows for signal processing
        - Uses 'open' price as entry price (not 'close')
        - Filters data up to current date to prevent future data leakage
        """
        signals = []
        ...
        for stock in stocks:
            try:
                ...
                # Match backtest_offline: Take last 5 rows, then get the latest
                last_5 = signal_df.iloc[-5:]
                latest = last_5.iloc[-1]
                
                # Check for buy signal (numeric comparison, matching backtest_offline exactly)
                if latest['signal'] == 1:
                    ...
                    signals.append({
                        ...
                        'open': float(latest['open']),  # Use OPEN price (not close)
    ...
    def scan_for_signals_not_used(self, symbols: List[str]) -> Dict[str, Any]:
                        ...
                        if all_signals:
                            ...
                            if self.can_open_position():
                                for signal in selected_signals:
                                    ...
                                    # Use 'open' price for entry (matching backtest_offline)
                                    signal['price'] = signal['open']
